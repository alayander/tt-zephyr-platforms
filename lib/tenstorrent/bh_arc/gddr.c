...
int LoadMriscFw(uint8_t gddr_inst, const uint8_t *tag)
{
	...
	rc = spi_arc_dma_transfer_to_tile(flash, tag, buf, TEMP_SPI_BUFFER_SIZE, (uint8_t *)mrisc_l1);
...
int LoadMriscFwCfg(uint8_t gddr_inst, const uint8_t *tag)
{
	...
	volatile uint32_t *mrisc_l1 = SetupMriscL1Tlb(gddr_inst);
	rc = spi_arc_dma_transfer_to_tile(flash, tag, buf, TEMP_SPI_BUFFER_SIZE, (uint8_t *)mrisc_l1 + MRISC_FW_CFG_OFFSET);
